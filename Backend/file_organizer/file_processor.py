...
def process_file(file_path: str, folder_id: int):
    try:
        ...
        if file_size_mb > max_size_mb:
            logger.warning(
                f"Skipping file {path_obj.name}: Size {file_size_mb:.2f}MB exceeds limit"
            )
            return {
                "status": "skipped",
                "file_name": path_obj.name,
        ...
        if raw_ft in ("image", "images", "img", "picture", "photo"):
            key = "images"
        elif raw_ft in ("video", "movies", "mp4", "mov"):
            key = "video"
        elif raw_ft in ("audio", "sound", "mp3", "wav"):
            key = "audio"
        else:
            key = "text"
        try:
            ...
            if create_backups:
                try:
                    ...
                    if backup_path.exists():
                        ...
                        backup_path = (
                            backup_dir / f"{path_obj.stem}_{ts}{path_obj.suffix}"
                        )
