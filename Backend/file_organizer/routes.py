...
from .file_watcher import stop_watching, start_all_watchers
...
def register_routes(app):
    ...
    def update_settings():
        ...
        return (
            jsonify({"success": True, "settings": settings})
            if success
            else (jsonify({"error": "Failed"}), 500)
        )
                ...
                for f in RAG_DATA_DIR.iterdir():
                    if f.is_file() and f.suffix == ".json":
                        try:
                            ...
                            logger.warning(f"Could not delete RAG file {f}: {e}")
    ...
    def handle_watched_folders():
        if request.method == "POST":
            ...
            new_folder = {
                ...
                "path": data["path"],
                "status": "Active",
                "fileCount": count_files_in_folder(data["path"]),
                "lastActivity": "Never",
    ...
    def process_folder_route(folder_id):
        ...
        for f in folder_path.iterdir():
            if f.is_file():
                if skip_hidden and is_hidden_check(f):
                    continue
                files.append(str(f))
    ...
    def get_progress(folder_id):
        return jsonify(
            state.processing_progress.get(
                folder_id, {"total": 0, "completed": 0, "failed": 0, "in_progress": 0}
            )
        )
    ...
    def search_rag():
        ...
        return jsonify(
            {
                "results": RAGGenerator.search_rag(
                    data.get("query", ""), data.get("max_results", 5)
                )
            }
        )
    ...
    def get_history_route():
        ...
        for entry in history:
            entry["timeAgo"] = calculate_time_ago(entry.get("timestamp", 0))
    ...
    def undo_movement_route(movement_id):
        ...
        if not movement or movement.get("status") == "undone":
    ...
    def open_location(movement_id):
        ...
        current_path = (
            movement["toPath"]
            if movement.get("status") == "completed"
            else movement["fromPath"]
        )
