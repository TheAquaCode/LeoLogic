...
import { MessageSquare, X, Maximize2, Minimize2, Send, Mic, ChevronRight, LayoutDashboard, FolderOpen, Upload, Clock, Settings } from 'lucide-react';
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="w-60 bg-white border-r border-gray-200 flex flex-col">
        <div className="p-4 border-b border-gray-200">
          <div className="flex items-center space-x-2">
            ...
            <div>
              <h1 className="font-semibold text-sm">AI File Sorter</h1>
              <p className="text-xs text-gray-500">Organize with AI</p>
            </div>
          </div>
        </div>
        
        <nav className="flex-1 p-4 space-y-1">
          <button className="w-full flex items-center space-x-3 px-3 py-2 rounded-lg bg-gray-100 text-gray-900">
      ...
      <div 
        ...
        style={{ 
          marginRight: isOpen && isMaximized ? '480px' : '0',
        }}
      >
        <div className="p-8">
          <div className="mb-6">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Dashboard</h1>
            <p className="text-gray-600">AI-powered file organization overview</p>
          </div>

          {/* Organization Progress */}
          <div className="bg-white rounded-lg shadow p-6 mb-6">
            <h2 className="text-lg font-semibold mb-4">Organization Progress</h2>
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm text-gray-600">Overall Organization</span>
          ...
          <div className="grid grid-cols-4 gap-4 mb-6">
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <div className="text-3xl font-bold mb-2">2,081</div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-semibold">341</span>
              </div>
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
        ...
        >
          ...
          >
            ...
            <div className="flex items-center space-x-1">
              <button onClick={toggleMaximize} className="p-1.5 hover:bg-gray-800 rounded transition-colors">
                {isMaximized ? <ChevronRight className="w-5 h-5" /> : <Maximize2 className="w-4 h-4" />}
              </button>
              <button onClick={() => setIsOpen(false)} className="p-1.5 hover:bg-gray-800 rounded transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
          </div>
              ...
              <div key={message.id} className={`flex ${message.type === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`flex items-start space-x-3 max-w-[85%] ${message.type === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`}>
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 ${
                    ...
                    {message.type === 'ai' ? <MessageSquare className="w-5 h-5" /> : <div className="w-5 h-5 rounded-full bg-white"></div>}
                  </div>
                  <div className={`flex flex-col ${message.type === 'user' ? 'items-end' : 'items-start'}`}>
                    <div className={`rounded-2xl px-4 py-3 ${message.type === 'ai' ? 'bg-gray-100 text-gray-900' : 'bg-blue-600 text-white'}`}>
