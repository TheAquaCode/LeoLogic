...
def start_watching(folder_id: int, folder_path: str):
    ...
    state.observers[folder_id] = {"observer": observer, "handler": event_handler}
...
def start_all_watchers():
    ...
    stop_scanner.set()
    if scanner_thread_obj and scanner_thread_obj.is_alive():
        scanner_thread_obj.join(timeout=1.0)
    stop_all_watchers()
    stop_scanner.clear()
    ...
    if frequency == "Real-time":
        ...
        scanner_thread_obj = threading.Thread(
            target=periodic_scan_loop, args=(interval,), daemon=True
        )
