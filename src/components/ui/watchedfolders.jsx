// src/components/ui/watchedfolders.jsx - FIXED: Only show progress for correct folder
...
const WatchedFolders = ({ 
  ...
  const getProgressPercent = () => {
    if (!processingProgress.total || processingProgress.total === 0) return 0;
        ...
        {safeFolders.length === 0 ? (
          ...
          <div className="space-y-3 overflow-y-auto h-full">
            {safeFolders.map((folder) => (
              <div key={folder.id} className="bg-white rounded-lg border border-gray-200 relative overflow-visible">
                {/* Progress bar at TOP - above everything */}
                {(() => {
                  const prog = processingProgress?.[folder.id] || {};
                  const pct = prog && prog.total ? Math.round((prog.completed || 0) / (prog.total || 1) * 100) : 0;
                  return processingFolder === folder.id && (prog.total > 0) ? (
                ...
                <div className="p-4 relative">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="relative flex-shrink-0 ml-2">
                      <button 
                        ...
                        className="text-gray-400 hover:text-gray-600 p-1 relative z-50"
                      ...
                      {openMenuId === folder.id && (
                        <>
                          ...
                          />
                          <div className="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-[110]">
                            {backendOnline && (
                              <button
                                onClick={() => {
                  ...
                  {/* Processing overlay - ONLY for this specific folder */}
                  {(() => {
                    const prog = processingProgress?.[folder.id] || {};
                    return processingFolder === folder.id ? (
                      <div className="absolute inset-0 bg-blue-50 bg-opacity-90 rounded-lg flex items-center justify-center pointer-events-none z-10">
                        <div className="text-center">
                          <Loader className="w-6 h-6 animate-spin text-blue-600 mx-auto mb-2" />
                          <p className="text-sm font-medium text-blue-900">
                            {prog.total > 0 
                              ? `Processing ${prog.completed || 0} of ${prog.total} files`
                              : 'Processing files...'}
                          </p>
                          {prog.failed > 0 && (
